...
def test_semantic_memory_only_mode_no_retrieved_facts() -> None:
    state = _build_state("what is my name?", summary_text="")
    node = _NoLLMNode()
    result = node.run(state)

    assert result.state.metadata["response_memory_explicit_query"] is False
    assert result.state.metadata["response_memory_relevant"] is False
    assert result.state.metadata.get("response_memory_match") is None
    assert result.state.response is not None
    assert result.state.response.final_answer == "what is my name?"